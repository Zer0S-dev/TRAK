...
void motionBegin() {
  ...
  devLog(String("GYRO | READY | sensitivity=") + String(sensitivityLevel) + " | threshold=" + String(thresholdForLevel(sensitivityLevel), 1) + " dps");
...
void motionUpdate(uint32_t now) {
  ...
  if (state.motionDps >= motionThresholdDps) {
    ...
    if (state.stationaryConfirmed || now - motionAboveThresholdSince >= GYRO_MOTION_CONFIRM_SEC * 1000UL) {
      ...
      if (!wasMoving) {
        devLog(String("MOTION | IMMOBILE->MOBILE | gyro=") + String(state.motionDps, 2) + " dps | threshold=" + String(motionThresholdDps, 1) + " dps");
    ...
    if (state.stationarySince == 0) { state.stationarySince = now; devLog(String("MOTION | MOBILE->CALM | gyro=") + String(state.motionDps, 2) + " dps | confirm=8s"); DevSerial.println("[MODE] RETOUR AU CALME - confirmation repos 8s demarree"); }
