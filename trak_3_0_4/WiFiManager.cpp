...
void wifiNetworkTick(bool){const uint32_t now=millis();if(active){if(WiFi.status()!=WL_CONNECTED){wifiLostSince=now;}else if(now-lastInternetCheck>=WIFI_INTERNET_CHECK_MS){lastInternetCheck=now;lastInternetResult=httpReachable();if(!lastInternetResult){if(wifiLostSince==0)wifiLostSince=now;}else wifiLostSince=0;}if(wifiLostSince!=0&&now-wifiLostSince>=WIFI_LOSS_CONFIRM_MS){Serial.println("[NET] Internet Wi-Fi perdu -> bascule 4G.");active=false;activeSlot=-1;WiFi.disconnect(false,false);wifiLostSince=0;}return;}finishReturnScan();if(active)return;if(!scanRunning&&now-lastReturnScan>=WIFI_RETURN_SCAN_MS){lastReturnScan=now;startReturnScan();}}
bool wifiPostJson(const String& json,int& httpStatus){httpStatus=0;if(!wifiIsActive())return false;WiFiClientSecure client;client.setInsecure();HTTPClient http;String url=String(TRAK_CONNECT_URL)+"?api_key="+TRAK_API_KEY;if(!http.begin(client,url))return false;http.setTimeout(8000);http.addHeader("Content-Type","application/json");httpStatus=http.POST(json);http.end();return httpStatus>=200&&httpStatus<300;}
bool wifiSyncProfilesFromServer(){if(!wifiIsActive())return false;WiFiClientSecure client;client.setInsecure();HTTPClient http;String url=String(WIFI_API_URL)+"?api_key="+TRAK_API_KEY;if(!http.begin(client,url))return false;http.setTimeout(6000);const int status=http.GET();if(status<200||status>=300){http.end();return false;}const String body=http.getString();http.end();for(uint8_t slot=0;slot<MAX_WIFI_PROFILES;++slot){const int pos=body.indexOf(String("\"slot\":")+slot);if(pos<0){Serial.printf("[WIFI] Profil #%u absent du dashboard -> profil local conserve.\n",slot+1);continue;}const int ssidKey=body.indexOf("\"ssid\":\"",pos),pwdKey=body.indexOf("\"password\":\"",pos);if(ssidKey<0||pwdKey<0)continue;const int ss=ssidKey+8,se=body.indexOf('"',ss),ps=pwdKey+12,pe=body.indexOf('"',ps);if(se<0||pe<0)continue;profiles[slot].ssid=body.substring(ss,se);profiles[slot].password=body.substring(ps,pe);saveProfile(slot);}Serial.println("[WIFI] Profils dashboard synchronises.");return true;}
