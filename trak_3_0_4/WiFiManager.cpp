...
constexpr char WIFI_API_URL[] = "https://surlereservoir.fr/trak/api/wifi/";
...
int findVisibleSlot(int count) {
  ...
  for (uint8_t slot = 0; slot < MAX_WIFI_PROFILES; ++slot) {
    ...
    for (int i = 0; i < count; ++i) {
      if (WiFi.SSID(i) == profiles[slot].ssid) return slot;
    }
  }
  return -1;
}
...
bool connectSlot(uint8_t slot) {
  ...
  Serial.printf("[WIFI] Connexion profil #%u : %s\n", slot + 1, profiles[slot].ssid.c_str());
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.printf("[WIFI] Echec profil #%u.\n", slot + 1);
    return false;
  }
  activeSlot = slot;
  active = true;
  wifiLostSince = 0;
  Serial.printf("[WIFI] Connecte : %s | IP %s\n", profiles[slot].ssid.c_str(), WiFi.localIP().toString().c_str());
  return true;
}
...
bool httpReachable() {
  ...
  IPAddress ip;
  if (WiFi.hostByName("surlereservoir.fr", ip) != 1) return false;
  WiFiClient client;
  client.setTimeout(1200);
  const bool ok = client.connect(ip, 443);
  client.stop();
  return ok;
}
...
void startReturnScan() {
  ...
  if (result == WIFI_SCAN_RUNNING) {
    ...
    WiFi.scanDelete();
    if (slot >= 0) connectSlot(slot);
  }
}

void finishReturnScan(bool cellularAvailable) {
  ...
  WiFi.scanDelete();
  if (slot < 0) return;
  if (connectSlot(static_cast<uint8_t>(slot)) && wifiInternetAvailable()) {
    active = true;
    return;
  }
  active = false;
  activeSlot = -1;
  if (cellularAvailable) Serial.println("[NET] Wi-Fi visible mais Internet non valide -> 4G conservee.");
}
...
uint8_t wifiProfileCount() {
  uint8_t count = 0;
  for (uint8_t i = 0; i < MAX_WIFI_PROFILES; ++i) if (validSlot(i)) ++count;
  return count;
}
...
bool wifiConnectBestSaved() {
  ...
  if (!connectSlot(static_cast<uint8_t>(slot))) return false;
  if (!wifiInternetAvailable()) {
    Serial.println("[WIFI] Wi-Fi connecte mais Internet indisponible.");
    WiFi.disconnect(false, false);
    active = false;
    activeSlot = -1;
    return false;
...
void wifiNetworkTick(bool cellularAvailable) {
  ...
  if (active) {
    if (WiFi.status() != WL_CONNECTED || !httpReachable()) {
      if (wifiLostSince == 0) wifiLostSince = now;
      if (now - wifiLostSince >= WIFI_LOSS_CONFIRM_MS) {
        Serial.println("[NET] Internet Wi-Fi perdu -> bascule 4G.");
        active = false;
        activeSlot = -1;
        WiFi.disconnect(false, false);
      }
  ...
  if (!cellularAvailable || wifiProfileCount() == 0) return;
  finishReturnScan(cellularAvailable);
  if (active) return;
...
bool wifiPostJson(const String& json, int& httpStatus) {
  ...
  client.setInsecure();
  HTTPClient http;
  String url = String(TRAK_CONNECT_URL) + "?api_key=" + TRAK_API_KEY;
  if (!http.begin(client, url)) return false;
  http.setTimeout(8000);
  http.addHeader("Content-Type", "application/json");
  httpStatus = http.POST(json);
  http.end();
...
bool wifiSyncProfilesFromServer() {
  ...
  client.setInsecure();
  HTTPClient http;
  String url = String(WIFI_API_URL) + "?api_key=" + TRAK_API_KEY;
  if (!http.begin(client, url)) return false;
  http.setTimeout(6000);
  const int status = http.GET();
  if (status < 200 || status >= 300) { http.end(); return false; }
  const String body = http.getString();
  http.end();

  for (uint8_t slot = 0; slot < MAX_WIFI_PROFILES; ++slot) {
    const String sKey = String("\"slot\":") + slot;
    const int pos = body.indexOf(sKey);
    if (pos < 0) { profiles[slot].ssid = ""; profiles[slot].password = ""; saveProfile(slot); continue; }
    const int ssidKey = body.indexOf("\"ssid\":\"", pos);
    const int pwdKey = body.indexOf("\"password\":\"", pos);
    if (ssidKey < 0 || pwdKey < 0) continue;
    const int ss = ssidKey + 8;
    const int se = body.indexOf('"', ss);
    const int ps = pwdKey + 12;
    const int pe = body.indexOf('"', ps);
    if (se < 0 || pe < 0) continue;
    profiles[slot].ssid = body.substring(ss, se);
    profiles[slot].password = body.substring(ps, pe);
    saveProfile(slot);
  }
  Serial.println("[WIFI] Profils dashboard synchronises.");
  return true;
}
