...
bool httpReachable(){if(WiFi.status()!=WL_CONNECTED)return false;WiFiClientSecure client;client.setInsecure();HTTPClient http;if(!http.begin(client,trakWebAppUrl()))return false;http.setTimeout(1500);const int status=http.GET();http.end();return status>0;}
...
bool wifiPostJson(const String& json,int& httpStatus){httpStatus=0;if(!wifiIsActive())return false;WiFiClientSecure client;client.setInsecure();HTTPClient http;const String url=trakWebAppUrl()+WIFI_API_PATH;if(!http.begin(client,url))return false;http.setTimeout(8000);http.addHeader("Content-Type","application/json");httpStatus=http.POST(json);http.end();return httpStatus>=200&&httpStatus<300;}
bool wifiSyncProfilesFromServer(){if(!wifiIsActive())return false;WiFiClientSecure client;client.setInsecure();HTTPClient http;const String url=trakWebAppUrl()+WIFI_API_PATH;if(!http.begin(client,url))return false;http.setTimeout(6000);const int status=http.GET();if(status<200||status>=300){http.end();return false;}const String body=http.getString();http.end();for(uint8_t slot=0;slot<MAX_WIFI_PROFILES;++slot){const int pos=body.indexOf(String("\"slot\":")+slot);if(pos<0){wifiClearProfile(slot);continue;}const int ssidKey=body.indexOf("\"ssid\":\"",pos),pwdKey=body.indexOf("\"password\":\"",pos);if(ssidKey<0||pwdKey<0)continue;const int ss=ssidKey+8,se=body.indexOf('"',ss),ps=pwdKey+12,pe=body.indexOf('"',ps);if(se<0||pe<0)continue;profiles[slot].ssid=body.substring(ss,se);profiles[slot].password=body.substring(ps,pe);saveProfile(slot);}Serial.println("[WIFI] Profils dashboard synchronises.");return true;}
