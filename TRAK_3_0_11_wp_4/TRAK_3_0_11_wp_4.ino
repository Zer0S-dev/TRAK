...
void setup()
{
  ...
  DevSerial.println();
  DevSerial.println("======================================");
  DevSerial.println(" TRAK - 3.0.11 / TrackingEngine v2");
  ...
  DevSerial.println(" TRAK Connect : WebSocket + JSON (TEST)");
  DevSerial.println("======================================");
  ...
  // Doit démarrer après trakConnectBegin() (qui charge l'URL API en cache)
  // et avant que trackingEngineBegin()/CommunicationManager ne produisent
  // du trafic cellulaire : ModemUplink est désormais le seul propriétaire
  // du modem 4G pour Trackserver ET TRAK Connect.
  modemUplinkBegin();
