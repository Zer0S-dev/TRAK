...
const TRAK_REGISTRY_FILE = TRAK_STORAGE_DIR . '/traks.json';
...
function ensureTrakStorage(): void
{
    ...
    if (!is_file(TRAK_REGISTRY_FILE)) {
        if (@file_put_contents(TRAK_REGISTRY_FILE, "{}", LOCK_EX) === false) {
            jsonResponse(['ok' => false, 'error' => 'registry_init_failed'], 503);
        }
    }
}
...
function loadTrakRegistry(): array
{
    ensureTrakStorage();
    $raw = @file_get_contents(TRAK_REGISTRY_FILE);
    if ($raw === false || trim($raw) === '') return [];
    $data = json_decode($raw, true);
    return is_array($data) ? $data : [];
}

function saveTrakRegistry(array $registry): void
{
    ensureTrakStorage();
    $json = json_encode($registry, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
    if ($json === false || @file_put_contents(TRAK_REGISTRY_FILE, $json, LOCK_EX) === false) {
        jsonResponse(['ok' => false, 'error' => 'registry_write_failed'], 503);
    }
}

function validTrakApiKey(string $key): bool
{
    return strlen($key) >= 32 && strlen($key) <= 128 && preg_match('/^[A-Za-z0-9_-]+$/', $key) === 1;
}

function requireApiKey(?string $trakId = null): void
{
    ...
    if ($trakId !== null && $trakId !== '') {
        if (!validTrakApiKey($provided)) jsonResponse(['ok' => false, 'error' => 'unauthorized'], 401);

        $registry = loadTrakRegistry();
        if (!isset($registry[$trakId])) {
            // First contact: the key generated by the TRAK is registered server-side.
            // This is a trust-on-first-use bootstrap; subsequent requests are checked.
            $registry[$trakId] = [
                'api_key' => $provided,
                'active' => true,
            ...
            saveTrakRegistry($registry);
            return;
        }

        $entry = $registry[$trakId];
        if (($entry['active'] ?? true) !== true || !hash_equals((string) ($entry['api_key'] ?? ''), $provided)) {
            jsonResponse(['ok' => false, 'error' => 'unauthorized'], 401);
        }

        $registry[$trakId]['last_seen_at'] = gmdate('Y-m-d\\TH:i:s\\Z');
        saveTrakRegistry($registry);
        return;
    }

    if (!hash_equals(TRAK_API_KEY, $provided)) jsonResponse(['ok' => false, 'error' => 'unauthorized'], 401);
}
