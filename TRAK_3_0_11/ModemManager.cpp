...
{
  // IMPORTANT : ne jamais donner un mutex que la tache courante
  // n'a pas effectivement acquis.
  if (modemMutex != nullptr) {
...
bool readGNSS(GnssData& data)
{
  ...
  // +CGNSSINFO fournit l'heure UTC juste après la date (HHMMSS.sss).
  if (tokenCount > 10 && tokens[10] && strlen(tokens[10]) >= 6) {
    const char* utc = tokens[10];
    if (isdigit(utc[0]) && isdigit(utc[1]) &&
        isdigit(utc[2]) && isdigit(utc[3]) &&
        isdigit(utc[4]) && isdigit(utc[5])) {
    ...
    uint64_t& epoch)
{
  ...
  // Howard Hinnant - days from civil, adapté en C++ embarqué.
    ...
    uint8_t hour, uint8_t minute, uint8_t second, uint16_t millisecond)
{
  ...
  if (gnssTimeToUnixSeconds(year, month, day, hour, minute, second, epoch)) {
    // Trackserver/OsmAnd attend timestamp={2} en temps Unix.
    char epochText[24];
    ...
    uint32_t& bytesSent)
{
  ...
  while (!client.available() && millis() - start < 3000) {
    // Le rendu LED continue pendant l'attente de la réponse HTTP Wi-Fi.
    statusLedService();
    delay(1);
  }
  ...
  if (success) {
    DevSerial.println(">> Success (Wi-Fi)");
  } else {
    ...
    uint32_t& bytesSent)
{
  ...
  //
  // IMPORTANT : toute la transaction HTTP est protégée par le mutex
  // transactionnel. Le mutex A7670 est récursif : sendATCommand() peut
  // donc être utilisé ici sans libérer le modem entre deux commandes.
  ...
  if (!httpReady) {
    ...
    // Conserver la réponse brute du modem : elle est indispensable pour
    // distinguer HTTP déjà initialisé, erreur de contexte PDP, erreur modem,
    // ou simple timeout de réponse.
    ...
    if (initRes.indexOf("OK") < 0) {
      ...
      // Nettoyage non bloquant de l'éventuelle session HTTP résiduelle.
      // On ne retente PAS HTTPINIT ici : le prochain cycle permettra de
      // déterminer si HTTPTERM suffit ou si le problème vient du PDP/modem.
...
bool connectCellularNetwork()
{
  ...
  // CGDCONT + CGACT forment une seule transaction modem.
  if (!lockModem(15000)) {
...
bool sendSMS(const char* phoneNumber, const char* message)
{
  ...
  if (phoneNumber == nullptr || message == nullptr ||
      strlen(phoneNumber) == 0 || strlen(message) == 0) {
    return false;
  }
  ...
  // Le A7670 peut occasionnellement refuser une seconde emission SMS
  // rapprochee. On garde le mutex pendant toute la transaction et on
  // autorise une seconde tentative apres abandon/nettoyage de la session.
  for (uint8_t attempt = 1; attempt <= 2 && !success; ++attempt) {
    ...
    if (response.indexOf("OK") < 0) {
      ...
      DevSerial.println(").");
      modemSerial.write((uint8_t)27); // ESC: annule une eventuelle session SMS
      delay(100);
      while (modemSerial.available()) modemSerial.read();
      continue;
    }

    // Jeu de caracteres GSM 7-bit pour les SMS texte basiques de Sentinel.
    ...
    modemSerial.write((uint8_t)26); // CTRL+Z
  ...
  if (success) {
    DevSerial.println("[SMS] Envoye.");
  } else {
    DevSerial.println("[SMS] Echec envoi.");
  }
...
bool readIncomingSMS(String& sender, String& message)
{
  ...
  // On ne lit que les SMS non lus. Le premier trouve est traite.
  ...
  // Format A7670 typique :
  // +CMGL: <index>,"REC UNREAD","<sender>",... 
  ...
  // Le numero expediteur est le champ entre les guillemets apres REC UNREAD.
  sender = response.substring(thirdQuote + 1, fourthQuote);

  // Le corps est sur la ligne suivante apres l'en-tete.
  ...
  while (bodyStart < static_cast<int>(response.length()) &&
  ...
  if (messageIndex >= 0) {
    ...
    while (millis() - start < 2500) {
      ...
      if (delResponse.indexOf("OK") >= 0 ||
          delResponse.indexOf("ERROR") >= 0 ||
          delResponse.indexOf("+CMS ERROR") >= 0) {
        break;
      }
      delay(1);
    }
