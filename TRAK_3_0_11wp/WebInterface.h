...
struct WebTrackerData {
  ...
  float motionXDps = 0.0f;
  float motionYDps = 0.0f;
  float motionZDps = 0.0f;
  float motionDps = 0.0f;
  bool moving = false;
  bool stationaryConfirmed = false;
  uint32_t motionStationaryConfirmationRemainingMs = 0;
  bool motionCalibrated = false;
  uint8_t motionSensitivityLevel = 3;
  float motionSensitivityThresholdDps = 6.0f;
  uint8_t sendIntervalLevel = 3;
  uint32_t sendIntervalMovingSec = 15;
  ...
  bool sentinelPhoneConfigured = false;
  const char* sentinelState = "OFF";
  uint32_t sentinelConfirmationRemainingMs = 0;
};
