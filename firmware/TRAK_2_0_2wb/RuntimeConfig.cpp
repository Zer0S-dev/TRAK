...
NetworkMode currentMode = NetworkMode::WIFI;
DataUsage dataUsage;
...
void runtimeConfigBegin()
{
  ...
  const uint8_t stored = prefs.getUChar("netmode", DEV_MODE ? 1 : 0);
  currentMode = (stored == 0) ? NetworkMode::CELLULAR : NetworkMode::WIFI;
...
bool isWiFiMode() { return currentMode == NetworkMode::WIFI; }

void setNetworkMode(NetworkMode mode)
{
  currentMode = mode;
  if (lockConfig()) {
    prefs.putUChar("netmode", mode == NetworkMode::WIFI ? 1 : 0);
    unlockConfig();
  }
}
